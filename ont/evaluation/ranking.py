...
def compute_metrics(ranks, num_entities):
    """Compute H@1, H@10, H@100, MRR, MR, median, AUC from rank tensor."""
...
def compute_rank_roc(ranks_dict, num_classes):
    """Compute ROC AUC from rank distribution."""
...
def dists_to_ranks(dists, targets):
    """Convert distance matrix to ranks for target indices."""
...
def combine_rankings(rankings, num_classes):
    """Combine multiple RankingResult objects."""
...
class RankingResult:
    ...
    def __str__(self):
        return (
            f"top1: {self.top1 / len(self):.2f}, top10: {self.top10 / len(self):.2f}, "
            f"top100: {self.top100 / len(self):.2f}, mean: {round(np.mean(self.ranks))}, "
            f"median: {round(np.median(self.ranks))}, "
